...
class Line_Marker(object):
    ...
    def write_to_rosbag(self, bag_name):
        print("writing marker to rosbag (square)...")
    ...
    def read_from_rosbag_and_publish(self, bag_name):
        print("reading marker from rosbag (square)...")
        msg_from_bag = None
...
wpose.position.y += scale * edge_len # 2st move sideways (y)
waypoints.append(copy.deepcopy(wpose))
line_marker.add_new_point(wpose.position.x, wpose.position.y, wpose.position.z)

wpose.position.x += scale * edge_len # 3rd move fwd/backwards in (x)
waypoints.append(copy.deepcopy(wpose))
line_marker.add_new_point(wpose.position.x, wpose.position.y, wpose.position.z)

wpose.position.y -= scale * edge_len # 4th move sideways (y)
waypoints.append(copy.deepcopy(wpose))
line_marker.add_new_point(wpose.position.x, wpose.position.y, wpose.position.z)

wpose.position.x -= scale * edge_len # 5th move fwd/backwards in (x)
waypoints.append(copy.deepcopy(wpose))
line_marker.add_new_point(wpose.position.x, wpose.position.y, wpose.position.z)
...
## --- ROSBAG CODE (WRITE): ----
print("writing trajectory to rosbag (square)...")
bag = rosbag.Bag('bag_msgs_square/display_trajectory_square.bag', 'w')
try:
...
print("reading trajectory from rosbag (square)...")
msg_from_bag = None
square_bag = rosbag.Bag('bag_msgs_square/display_trajectory_square.bag')
# read all messages from the indicated topics:
for topic, msg, t in square_bag.read_messages(topics=['/move_group/display_planned_path']):
...
## ONLY USE THE BELOW CODE WHEN RUNNING .PY WITHIN SCRIPS DIRECTORY!
# (env in shbang does not have the necessary bag_msg_<shape> folders)
line_marker.write_to_rosbag('bag_msgs_square/marker_bag_square.bag')
line_marker.read_from_rosbag_and_publish('bag_msgs_square/marker_bag_square.bag')
'''
